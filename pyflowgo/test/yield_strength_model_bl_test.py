...
class MyTestCase(unittest.TestCase):
    def test_read_initial_condition_from_json_file(self):
        ...
        yield_strength_model_bl = pyflowgo.flowgo_yield_strength_model_bl.FlowGoYieldStrengthModelBirnbaumLev()
        yield_strength_model_bl.read_initial_condition_from_json_file(filename)
    ...
    def test_compute_yield_strength(self):
        ...
        yield_strength_model_bl = pyflowgo.flowgo_yield_strength_model_bl. \
            FlowGoYieldStrengthModelBirnbaumLev(vesicle_fraction_model=vesicle_fraction_model_constant)
        ...
        self.assertAlmostEqual(yield_strength, 9.207386317481683,10)

    def test_compute_basal_shear_stress(self):

        yield_strength_model_ryerson = pyflowgo.flowgo_yield_strength_model_ryerson.FlowGoYieldStrengthModelBirnbaumLev()
        ...
        state = pyflowgo.flowgo_state.FlowGoState()
        basal_shear_stress = yield_strength_model_bl.compute_basal_shear_stress(state, terrain_condition,
                                                                                   material_lava)

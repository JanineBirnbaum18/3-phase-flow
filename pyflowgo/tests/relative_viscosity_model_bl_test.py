...
class MyTestCase(unittest.TestCase):
    ...
    def test_compute_relative_viscosity(self):
        ...
        relative_viscosity_model_bl = pyflowgo.flowgo_relative_viscosity_model_bl. \
            FlowGoRelativeViscosityModelBirnbaumLev(vesicle_fraction_model=vesicle_fraction_model_constant)

        state = pyflowgo.flowgo_state.FlowGoState()
        state.set_crystal_fraction(0.15)

        relative_viscosity = relative_viscosity_model_bl.compute_relative_viscosity(state)
        self.assertAlmostEqual(relative_viscosity, 17.894752258258244,10) # model at phi = 0.15, phi_b = 0.64
